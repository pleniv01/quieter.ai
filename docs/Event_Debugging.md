...
## Applying this method to any GPT-style UI

For future GPT UIs (ChatGPT, Perplexity, Gemini, etc.), follow the same pattern you used for claude.ai:

1. **Locate the input element**
   * In DevTools Console, run:
     * `document.querySelectorAll('[data-testid="chat-input"], textarea, [contenteditable="true"]')`
   * Inspect the NodeList and pick the element whose text matches what you type.
   * Prefer selectors based on stable attributes (`data-testid`, `aria-label`, `role`), not brittle class names.

2. **Locate the submit trigger**
   * Find a nearby send button (`button[aria-label*="send" i]`, `button[type="submit"]`, etc.).
   * Or confirm Enter is handled on the input element itself.

3. **Hook both keyboard and click in a site-specific adapter**
   * In the extension content script for that site, always add:
     * `keydown` on the input (Enter without Shift) → `mirrorToQuieter(text)`.
     * `click` on the send button → `mirrorToQuieter(text)`.
   * `mirrorToQuieter(text)` should:
     * Read the input’s text (`innerText`/`textContent` or `value`).
     * Skip if empty or unchanged (last text).
     * Call `/query` with `{ prompt: text, metadata: { source: '<site-name-mirror>' } }`.

4. **Confirm via Network/Proxy once**
   * Use DevTools Network (and optionally Charles/Proxyman) to confirm that each submit:
     * Fires your handler.
     * Sends one clean `/query` with the expected `Authorization` header and prompt.

5. **Freeze the adapter**
   * Once confirmed, treat that adapter module as the single source of truth for that site.
   * If the provider changes their DOM, re-run steps 1–4 and update only that adapter.

This keeps GPT integrations repeatable: you always follow the same discovery steps, always use attribute-based selectors, and always route through a single mirror function that talks to Quieter.
